...
const TrainingCycleIndicator: React.FC<TrainingCycleIndicatorProps> = ({
  ...
  return (
    <div className="flex flex-col">
      ...
      {/* Always visible training logs */}
      {showLogs && (
        <div className="mt-3 bg-gray-900 rounded-md p-3 font-mono text-xs overflow-hidden border border-gray-800 animate-fade-down w-full max-w-md">
          ...
          <div className="h-40 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-gray-900 bg-black p-2 rounded">
            {trainingLogs.map((log, index) => (
              <div key={index} className="text-[10px] leading-tight mb-1">
                <span className="text-blue-400">Iteration {log.iteration}: </span>
            ...
            {trainingLogs.length === 0 && (
              <div className="text-gray-500 text-[10px]">No training data available yet...</div>
            )}
          </div>
        </div>
      )}
